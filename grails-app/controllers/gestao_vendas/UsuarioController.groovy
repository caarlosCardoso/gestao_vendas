...
class UsuarioController {
    ...
    def delete(Long id) {
        ...
        if (usuario.usuario == session.user) {
            flash.message = message(code: 'usuario.logado.message')
    ...
    def authLogin(){
        def usuario  = Usuario.createCriteria().get(){
            eq('usuario', params.usuario);
            eq('senha', params.senha);
        }

        if(usuario==null){
            flash.message = message(code: 'default.invalid.usuario.message')
            redirect(controller:'usuario', action:'login')
        }else{
            session.user = usuario.usuario
            redirect(controller:'venda', action:'index')
        }  
